...
### Health-Check (automatische Modus-Wahl)

```bash
# Health-Check ausfuehren
bash ${CLAUDE_PLUGIN_ROOT}/scripts/kroki-health.sh

# Output (verfuegbar):
# {"available": true, "endpoint": "http://localhost:8000", "latency_ms": 12}

# Output (nicht verfuegbar):
# {"available": false, "endpoint": "http://localhost:8000", "error": "HTTP 000", "fallback": "mermaid-js"}
```
...
Kroki Health-Check
    |
    +-- available: true  --> PRE-RENDER Mode
    |     - POST an $KROKI_ENDPOINT/{engine}/svg
    |     - SVG in assets/ speichern
    |     - 100% offline-faehig
    |     - Alle Engines verfuegbar
    |
    +-- available: false --> CLIENT-SIDE Mode
          - Mermaid.js CDN in HTML einbinden
          - Mermaid-Syntax inline in <pre class="mermaid">
          - NUR Mermaid-Syntax (kein D2, PlantUML etc.)
          - Benoetigt Internet fuer CDN
```

### Engine-Fallback-Matrix

| Engine (Kroki) | Client-Side Fallback | Qualitaetsverlust |
|----------------|---------------------|-------------------|
| Mermaid | Mermaid.js CDN | Keiner |
| D2 | Mermaid-Konvertierung (vereinfacht) | Mittel (kein Auto-Layout) |
| PlantUML | Mermaid-Konvertierung (vereinfacht) | Hoch (UML Features fehlen) |
| GraphViz | Nicht moeglich | Hoch → Textuelle Beschreibung |
| Excalidraw | Nicht moeglich | Hoch → Textuelle Beschreibung |
| BPMN | Nicht moeglich | Hoch → Mermaid Flowchart |
| C4PlantUML | Nicht moeglich | Hoch → Mermaid Flowchart |

**Empfehlung:** Bei Client-Side Mode NUR Mermaid-Syntax verwenden. Fuer andere Engines: Warnung ausgeben und auf Mermaid-kompatible Alternative umschreiben.

### Client-Side HTML Template (Mermaid.js Fallback)

```html
<!-- Mermaid.js Fallback (wenn Kroki nicht verfuegbar) -->
<script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>
<script>mermaid.initialize({startOnLoad: true, theme: 'neutral'});</script>

<section class="diagram">
    <h2>Diagramm-Titel</h2>
    <pre class="mermaid">
    flowchart LR
        A[Start] --> B[Ende]
    </pre>
    <p class="caption">Beschreibung des Diagramms</p>
</section>
```
