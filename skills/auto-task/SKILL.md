...
4b. Vault Status-Sync (optional): Wenn `obsidian.com version 2>/dev/null` erfolgreich → `obsidian.com property:set name="status" value="in_progress" type=text file="TASK-NNN-name"`. Bei Fehler: ignorieren (Vault-Sync ist Bonus, nicht kritisch).
...
Fuer schrittweise Ausfuehrung mit Budget-Intelligence und Subagent-Delegation:
- Sage "Arbeite TASK-XXX ab" → aktiviert `/task-orchestrator`
- Dort: User-Bestaetigung pro Phase, Budget-Checks, parallele Subagents
